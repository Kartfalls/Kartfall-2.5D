...
function sqrt5mod8(Fp, n2) {
  ...
  const nv = Fp.mul(n2, v3);
  const i3 = Fp.mul(Fp.mul(nv, _2n), v3);
...
function calcOffsets(n2, window, wOpts) {
...
var wNAF = class {
  ...
  wNAF(W, precomputes, n2) {
    ...
    for (let window = 0; window < wo.windows; window++) {
      const { nextN, offset, isZero, isNeg, isNegF, offsetF } = calcOffsets(n2, window, wo);
      n2 = nextN;
      if (isZero) {
  ...
  wNAFUnsafe(W, precomputes, n2, acc = this.ZERO) {
    ...
    for (let window = 0; window < wo.windows; window++) {
      if (n2 === _0n3)
        break;
      const { nextN, offset, isZero, isNeg } = calcOffsets(n2, window, wo);
      n2 = nextN;
      if (isZero) {
        continue;
      } else {
...
function weierstrassN(params, extraOpts = {}) {
  ...
  function pointToBytes2(_c, point, isCompressed) {
    ...
    if (isCompressed) {
      ...
      const hasEvenY = !Fp.isOdd(y3);
      return concatBytes(pprefix(hasEvenY), bx);
    } else {
      return concatBytes(Uint8Array.of(4), bx, Fp.toBytes(y3));
    }
  }
  function pointFromBytes(bytes) {
    ...
    if (length === comp && (head === 2 || head === 3)) {
      ...
      const y3 = Fp.fromBytes(tail.subarray(L2, L2 * 2));
      if (!isValidXY(x2, y3))
  ...
  class Point2 {
    ...
    static fromAffine(p3) {
      const { x: x2, y: y3 } = p3 || {};
      if (!p3 || !Fp.isValid(x2) || !Fp.isValid(y3))
        throw new Error("invalid affine point");
      if (p3 instanceof Point2)
        throw new Error("projective point not allowed");
      if (Fp.is0(x2) && Fp.is0(y3))
        return Point2.ZERO;
      return new Point2(x2, y3, Fp.ONE);
    }
...
function SWUFpSqrtRatio(Fp, Z) {
  ...
  const c7 = Fp.pow(Z, (c22 + _1n4) / _2n2);
  let sqrtRatio = (u3, v3) => {
    let tv1 = c6;
    let tv2 = Fp.pow(v3, c4);
    let tv3 = Fp.sqr(tv2);
    tv3 = Fp.mul(tv3, v3);
...
function mapToCurveSimpleSWU(Fp, opts) {
  ...
  if (!Fp.isValid(A2) || !Fp.isValid(B3) || !Fp.isValid(Z))
  ...
  return (u3) => {
    ...
    const e1 = Fp.isOdd(u3) === Fp.isOdd(y3);
...
var secp256k12 = createCurve({ ...secp256k1_CURVE, Fp: Fpk1, lowS: true, endo: secp256k1_ENDO }, sha256);
...
function lift_x(x2) {
  ...
  const c3 = Fp.create(xx * x2 + BigInt(7));
  let y3 = Fp.sqrt(c3);
  if (!hasEven(y3))
    y3 = Fp.neg(y3);
...
var SHA2562 = class extends HashMD2 {
  ...
  process(view, offset) {
    ...
    for (let i3 = 0; i3 < 64; i3++) {
      ...
      const T12 = H + sigma1 + Chi2(E, F2, G) + SHA256_K2[i3] + SHA256_W2[i3] | 0;
...
async function sign({ hash, privateKey, to = "object" }) {
  const { r: r4, s: s3, recovery } = secp256k1.sign(hash.slice(2), privateKey.slice(2), {
    lowS: true,
    extraEntropy: isHex(extraEntropy, { strict: false }) ? hexToBytes(extraEntropy) : extraEntropy
  });
  const signature = {
    r: numberToHex(r4, { size: 32 }),
...
//# sourceMappingURL=chunk-5Z4TAO57.js.map
