...
function getAddressLookupMetas(compiledAddressTableLookups, addressesByLookupTableAddress) {
  ...
  for (const lookup of compiledAddressTableLookups) {
    ...
    const readOnlyForLookup = readonlyIndexes.map((r2) => ({
      address: addresses[r2],
      addressIndex: r2,
      lookupTableAddress: lookup.lookupTableAddress,
      role: AccountRole.READONLY
    }));
...
var OffchainMessageContentFormat = ((OffchainMessageContentFormat3) => {
...
function getDefaultRequestTransformerForSolanaRpc(config) {
  ...
  return (request) => {
    return pipe(
      request,
      handleIntegerOverflow ? getIntegerOverflowRequestTransformer(handleIntegerOverflow) : (r2) => r2,
...
function getDefaultResponseTransformerForSolanaRpc(config) {
  return (response, request) => {
    ...
    return pipe(
      response,
      (r2) => getThrowSolanaErrorResponseTransformer()(r2, request),
      (r2) => getResultResponseTransformer()(r2, request),
      (r2) => getBigIntUpcastResponseTransformer(keyPaths ?? [])(r2, request)
    );
  };
}
function getDefaultResponseTransformerForSolanaRpcSubscriptions(config) {
  return (response, request) => {
    const methodName = request.methodName;
    const keyPaths = config?.allowedNumericKeyPaths && methodName ? config.allowedNumericKeyPaths[methodName] : void 0;
    return pipe(response, (r2) => getBigIntUpcastResponseTransformer(keyPaths ?? [])(r2, request));
  };
}
...
var o6 = globalThis.AbortController;
var PING_PAYLOAD = {
  jsonrpc: "2.0",
  method: "ping"
};
function getRpcSubscriptionsChannelWithAutoping({
  ...
  const pingerAbortController = new o6();
...
// node_modules/@privy-io/react-auth/dist/esm/use-sign-with-user-signer-Do5Oi_rb.mjs
var import_react = __toESM(require_react(), 1);
var r = () => {
  let { walletProxy: r2, client: a } = I();
  return (0, import_react.useMemo)((() => ({ signWithUserSigner: async ({ message: t4, targetAppId: e3 }) => {
    if (!r2) throw Error("Wallet proxy not initialized");
    let i = await a.getAccessToken();
    if (!i) throw Error("User must be authenticated");
    let { signature: s } = await r2.signWithUserSigner({ accessToken: i, message: t4, targetAppId: e3 });
    return { signature: s };
  } })), [r2, a]);
};
...
//# sourceMappingURL=chunk-HPVRE2BD.js.map
