...
function sha2563(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2562(from4(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
...
function weierstrass(curveDef) {
  ...
  const slcNum = (b2, from13, to) => bytesToNumberBE(b2.slice(from13, to));
  class Signature {
...
var import_index3 = __toESM(require_eventemitter32(), 1);
...
function getAuthenticatorData(options = {}) {
  ...
  const rpIdHash = sha2563(fromString3(rpId));
...
//# sourceMappingURL=dist-2KD4WQ3M.js.map
