...
import { FirstClassChainType, PasskeyAuthenticateRequestBody, OAuthProviderID, SmartWalletType, CustomOAuthProviderID, User as User$1, CoinbaseOnRampInitInput, CoinbaseOnRampInitResponse, CoinbaseOnRampStatusResponse, TransactionScanningRequestBody, TransactionScanningResponseBody } from '@privy-io/api-types';
...
import { P as PrivyEvents } from './events-context-BXis4NRh.js';
export { C as CallbackError } from './events-context-BXis4NRh.js';
...
/** Supported fiat currencies for onramp funding flows. */
type SupportedFiatCurrency = 'usd' | 'eur' | 'mxn' | 'brl' | 'gbp';
...
interface PrivyInterface {
    ...
    /**
     * For users who are authenticated, prompts the user to link an OAuth account.
     * Accepts any OAuth provider including built-in providers (e.g. 'google', 'twitter')
     * and custom OAuth providers (e.g. 'custom:my-provider').
     * This will directly initiate the OAuth flow for the given provider.
     *
     * @param options.provider The OAuth provider to link
     */
    linkOAuth: (options: {
    ...
    /**
     * Unlink an OAuth account from a user, by passing the provider and subject ID.
     * Accepts any OAuth provider including built-in providers (e.g. 'google', 'twitter')
     * and custom OAuth providers (e.g. 'custom:my-provider').
     * Note that you can only unlink an account if the user has at least one other account.
     *
     * @param options.provider The OAuth provider to unlink
     * @param options.subject The provider's subject ID for the account
     * @returns Promise for the {@link User} object after the account has been unlinked
     */
    unlinkOAuth: (options: {
        provider: OAuthProviderID;
        subject: string;
 ...
 * @returns linkOAuth - initiates the OAuth flow for any provider, including custom OAuth providers (e.g. 'custom:my-provider')
 *
 */
declare function useLinkAccount(callbacks?: PrivyEvents['linkAccount']): {
    ...
    linkPasskey: () => void;
    linkOAuth: (options: {
        provider: _privy_io_api_types.OAuthProviderID;
    }) => void;
};
...
type UseFundWalletWithBankDepositOptions = {
    ...
    source: {
         ...
         */
        assets: SupportedFiatCurrency[];
        /**
         * The default selected source asset.
         */
        defaultAsset?: SupportedFiatCurrency;
        /**
         * @deprecated
         *
